...
class Display(object):
    ...
    def __init__(self, spi, cs, dc, rst, width=256, height=64, text=""):
        ...
        self.write_cmd(self.SET_DISPLAY_OFFSET, 0x00) #was 00
        self.write_cmd(self.SET_DISPLAY_START_LINE, 0x00)
        # Column address 0 mapped to SEG0
        # Disable nibble remap
        # Scan from COM[N-1] to C0M0 *changed
        # Disable COM split between odd and even
        # Enable dual COM line mode
        self.write_cmd(self.SET_REMAP_DUAL_COM_LINE_MODE, 0x04, 0x11) #bill was 0x14,0x11 # 10.1.6
        ...
        self.write_cmd(self.SET_CONTRAST_CURRENT, 0x20) #bill 0x9F
        ...
        self.write_cmd(self.DISPLAY_ENHANCEMENT_B,
                       self.RESERVED_ENHANCEMENT | 0x00, 0x00) #bill 0xA2, 0x20)
        # Set pre-charge voltage level as 0.60 * VCC
        self.write_cmd(self.SET_PRECHARGE_VOLTAGE, 0x1F) #bill was 1f
        ...
        #self.write_cmd(self.SET_DISPLAY_MODE_INVERSE)
        self.write_cmd(self.PARTIAL_DISPLAY_DISABLE)
        self.write_cmd(self.DISPLAY_SLEEP_OFF)

        self.clear_buffers()
        if text:
            # put something on screen while loading fonts etc.
            self.draw_text8x8(0,0, text)
...
#             return True
    ...
    def present(self):
        ...
        """
        x0 = 4 #bill was 0
        x1 = 4 + (self.width // 4 - 1)  # bill was +0 2 bytes per address, 2 pixels per byte
        y0 = 0
        y1 = self.height - 1
